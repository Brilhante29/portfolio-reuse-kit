...
3. Use the selected pack to decide which skills, decision sources, SDD files, benchmark assets, and validation gates apply.
4. If `openspec/config.yaml` exists, treat it as the project planning config. If OpenSpec is not installed, still follow the same artifact graph manually in `sdd/`.
5. Keep planning artifacts iterative: update intent, portfolio impact, architecture, component pack, reuse delta, benchmark proof, tasks, and verification as implementation learns new facts.
6. Never install external components, MCPs, hooks, or settings automatically. Record the recommendation first and require explicit user approval for anything that changes machine-level tooling.
7. Every reusable discovery must end in one of three states: patch the kit now, record backlog, or reject with reason.
