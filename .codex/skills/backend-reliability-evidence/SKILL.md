...
description: Build or audit backend reliability repositories that claim transactions, event sourcing, sagas, outbox delivery, caching, consistency, or failure recovery. Use when Codex must replace in-memory simulations with real local infrastructure, connect repositories through versioned contracts, or publish reproducible failure benchmarks without false production claims.
---
