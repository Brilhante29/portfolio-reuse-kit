...
| 4 | `stroke-signal-demo` | medical-ml | python-3.12, numpy-2.5.1, scipy-1.18.0, jsonschema-4.26.0, docker | avaliacao de segmentacao CT sintetica com isolamento por paciente e sem selecao pelo teste | segmentation_dice, sensitivity, specificity, accuracy, confusion_matrix, latency_p95_ms |
| 5 | `alpr-mercosul` | ocr | python-3.12, pillow-10.4.0, numpy-1.26.4, pytest, docker | OCR image-only em placas Mercosul sinteticas com limite explicito | character_accuracy, plate_accuracy |
| 6 | `melanoma-classifier` | medical-vision | python-3.12.13, numpy-2.0.2, scipy-1.15.3, scikit-learn-1.7.2, dermamnist-v2.1, docker | avaliacao melanoma-vs-rest sem leakage no DermaMNIST oficial | auc, sensitivity, specificity, confusion_matrix |
...
| 8 | `embeddings-benchmark` | retrieval | python, fastembed-0.8.0, onnxruntime, bge-small-en-v1.5, all-minilm-l6-v2, sparse-vectorizers, jsonl, docker | comparacao de modelos de embedding | recall_at_k, indexing_time_ms, query_time_ms |
| 9 | `llm-agent-eval` | agents | python, jsonl, unittest, docker | agentes avaliados por tarefa | task_success_rate, cost_per_task |
| 10 | `prompt-ab-testing` | prompt-eval | python, bootstrap-statistics, jsonl, unittest, docker | comparacao estatistica de prompts | score_by_variant, confidence_interval |
...
| 15 | `grpc-vs-rest-bench` | protocol-benchmark | go, grpc, chi, protobuf, docker | comparacao REST vs gRPC sobre o mesmo contrato e payload | rest_over_grpc_p95_ratio, latency_ms_by_protocol, throughput_rps_by_protocol, parity_failures |
...
| 17 | `multi-tenant-starter` | backend | java21, spring-boot, spring-jdbc, postgresql17, flyway, testcontainers, docker | onboarding transacional e isolamento schema-per-tenant em PostgreSQL real | tenant_onboarding_p50_ms, tenant_onboarding_p95_ms, isolated_query_p95_ms, leakage_count |
| 18 | `api-gateway-lite` | gateway | go, reverse-proxy, redis, opentelemetry, k6, docker | gateway com auth, quota Redis compartilhada, propagacao de contexto e OTLP plugavel | overhead_p95_ms, gateway_throughput_rps, gateway_rejects, gateway_failures |
...
| 23 | `feature-store-lite` | data-platform | python-3.12, feast-0.64.0, pandas-2.3.3, pyarrow-25.0.0, parquet, sqlite, docker | ingestao fail-closed, point-in-time correto, TTL e materializacao offline-online | online_read_latency_p95_ms, online_entity_values_per_second |
