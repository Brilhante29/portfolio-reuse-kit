...
description: Design or audit local-first observability proof that correlates one real incident across metrics, traces, and logs. Use when Claude must instrument a service through OTLP, split fast evidence from a full Grafana exploration stack, measure recovery with a monotonic clock, preserve non-root Docker portability, or publish source-locked exact-head evidence.
---
