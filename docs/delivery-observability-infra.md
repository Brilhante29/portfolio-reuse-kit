...
| 25 | `observability-stack` | Correlate traces, metrics, and logs and measure simulated MTTR | Next |
| 27 | `terraform-aws-baseline` | Provision a local-first Kumo-compatible baseline with a plug-compatible AWS adapter | Pending |
...
flowchart LR
  ...
  observe --> evidence["Benchmark result V2"]
```
...
Each repository needs a credential-free local Docker path, README headline number, benchmark result V2, clean source provenance, and successful CI at the exact final `main`. Current progress is 1/4.
